...
MCP23017::MCP23017(int &bus, uint8_t address) {
    _deviceAddr = address;
	_bus = bus;
}
...
uint8_t MCP23017::activeDev = 0xFF;

void MCP23017::selectDev() {
    if (activeDev != _deviceAddr) {
        if (ioctl(_bus, I2C_SLAVE, _deviceAddr) < 0) {
            cout << "ERROR 2 HANDLING; you can check errno to see what went wrong" << endl;
            exit(1);
        }
    }
    activeDev = _deviceAddr;
}
...
void MCP23017::init()
{
    ...
    writeRegisterWord(MCP23017Register::GPPU_A, 0xFFFF);
}
...
{
    writeRegisterWord(MCP23017Register::GPIO_A, value);
}
...
void MCP23017::writeRegisterWord(MCP23017Register reg, uint16_t port)
{
    ...
    i2c_smbus_write_word_data(_bus, static_cast<uint8_t>(reg), port);
}
...
void MCP23017::readRegister(MCP23017Register reg, uint8_t& portA, uint8_t& portB)
{
    ...
    portA = (uint8_t)port & 0x00FF;
    portB = (uint8_t)((port & 0xFF00) >> 8);
}
